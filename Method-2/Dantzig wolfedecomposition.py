...
def check_size_dantzig(c, L, b0, A, b):
    ...
    row_lnk = len(b0)
    if n_blk != len(c) or n_blk != len(L) or n_blk != len(b)             or any(row_lnk != len(L[i]) for i in range(len(L))):
        return False
    elif not is_equal_list(col_blk, col_L) or not is_equal_list(col_blk, col_c)             or not is_equal_list(row_blk, row_b):
        return False
    else:
        return True
